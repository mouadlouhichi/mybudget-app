...
import { saveMonth, subscribeMonth, getMonth, getSavings, saveSavings, subscribeSavings, getDebts, saveDebts, subscribeDebts } from '@/lib/db'
import {
  emptyMonth, rolloverMonth, MonthBudget, VariableExpense, FixedExpense, SavingGoal, SavingsData, emptySavings,
  DebtItem, DebtKind, DebtsData, emptyDebts, debtRemaining, totalOutstanding,
...
import {
  ...
  Sliders, ArrowCircleUp, HandCoins, ArrowDown, ArrowUp,
...
function IconBadge({ Icon: Ico, color, size = 46 }: { Icon: Icon; color: string; size?: number }) {
  return (
    <div className="relative flex items-center justify-center flex-shrink-0" style={{ width: size, height: size }}>
      ...
      <div className="relative flex items-center justify-center rounded-full"
        ...
        <Ico size={size * 0.5} weight="bold" color={color} />
      </div>
    </div>
  )
}
  ...
  return (
    <Modal title="Settings" onClose={onClose}>
      ...
      <div style={{display:'flex',justifyContent:'center',gap:16}}>
        <a href="/privacy" style={{fontSize:11,color:'var(--t3)',fontWeight:600}}>Privacy Policy</a>
        <a href="/terms" style={{fontSize:11,color:'var(--t3)',fontWeight:600}}>Terms of Service</a>
      </div>
...
/* ══════════════════════ OVERVIEW ══════════════════════ */
function Overview({ month, savings, debts }: { month: MonthBudget; savings: SavingsData; debts: DebtsData }) {
  const owedByMe = totalOutstanding(debts.items, 'debt')
  const owedToMe = totalOutstanding(debts.items, 'credit')
  ...
  return (
    <div className="space-y-4 slide-up">
      ...
      {/* Debts & credits snapshot */}
      {(owedByMe>0||owedToMe>0)&&(
        <div className="glass md:col-span-1" style={{padding:20}}>
          <SectionHeader title="Debts &amp; Credits"/>
          <div style={{display:'flex',flexDirection:'column',gap:14}}>
            <div style={{display:'flex',alignItems:'center',gap:12}}>
              <IconBadge Icon={ArrowUp} color="var(--bad)"/>
              <div style={{flex:1}}>
                <p style={{fontSize:12,color:'var(--t2)'}}>You owe</p>
                <p className="f-display num" style={{fontSize:17,fontWeight:700,color:'var(--bad)'}}>{fmt(owedByMe)} MAD</p>
              </div>
            </div>
            <div style={{display:'flex',alignItems:'center',gap:12}}>
              <IconBadge Icon={ArrowDown} color="var(--good)"/>
              <div style={{flex:1}}>
                <p style={{fontSize:12,color:'var(--t2)'}}>Owed to you</p>
                <p className="f-display num" style={{fontSize:17,fontWeight:700,color:'var(--good)'}}>{fmt(owedToMe)} MAD</p>
              </div>
            </div>
          </div>
        </div>
              ...
              {cats.map(c=>{
                ...
                return (
                  <div key={c.type} className="flex flex-col items-center text-center">
                    <IconBadge Icon={catIcon(month, c.type)} color={categoryColor(month,c.type)} size={52}/>
...
/* ══════════════════════ DEBTS & CREDITS ══════════════════════ */
function AddDebtModal({ kind, onClose, onAdd }: { kind: DebtKind; onClose: () => void; onAdd: (d: Omit<DebtItem,'id'|'createdDate'|'paid'|'settled'>) => void }) {
  const [person, setPerson] = useState('')
  const [amount, setAmount] = useState('')
  const [dueDate, setDueDate] = useState('')
  const [note, setNote] = useState('')
  return (
    <Modal title={kind==='debt' ? 'New Debt' : 'New Credit'} onClose={onClose}>
      <div><FL label={kind==='debt' ? 'Who you owe' : 'Who owes you'}/><input className="field" placeholder="Name" value={person} onChange={e=>setPerson(e.target.value)} autoFocus/></div>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
        <div><FL label="Amount (MAD)"/><input className="field" type="number" placeholder="0" value={amount} onChange={e=>setAmount(e.target.value)}/></div>
        <div><FL label="Due date (optional)"/><input className="field" type="date" value={dueDate} onChange={e=>setDueDate(e.target.value)}/></div>
      </div>
      <div><FL label="Note (optional)"/><input className="field" placeholder="What's it for" value={note} onChange={e=>setNote(e.target.value)}/></div>
      <button className="btn-primary tap" onClick={()=>{ if(!person||!amount) return; onAdd({person,amount:parseFloat(amount),kind,dueDate:dueDate||undefined,note:note||undefined}); onClose() }}>
        <PlusCircle size={16} weight="bold"/> {kind==='debt' ? 'Add Debt' : 'Add Credit'}
      </button>
    </Modal>
  )
}

function PayDebtModal({ item, onClose, onPay }: { item: DebtItem; onClose: () => void; onPay: (amount: number) => void }) {
  const remaining = debtRemaining(item)
  const [amount, setAmount] = useState('')
  const amt = parseFloat(amount) || 0
  return (
    <Modal title={item.kind==='debt' ? `Pay back ${item.person}` : `Record from ${item.person}`} onClose={onClose}>
      <p style={{fontSize:12,color:'var(--t3)'}}>{fmt(remaining)} MAD remaining of {fmt(item.amount)} MAD</p>
      <div><FL label="Amount (MAD)"/><input className="field" type="number" placeholder="0" value={amount} onChange={e=>setAmount(e.target.value)} autoFocus/></div>
      <button className="btn-primary tap" onClick={()=>{ if(!amt) return; onPay(Math.min(amt,remaining)); onClose() }}>
        <Check size={16} weight="bold"/> {item.kind==='debt' ? 'Record Payment' : 'Record Collection'}
      </button>
    </Modal>
  )
}

function DebtsTab({ debts, onAdd, onDelete, onPay, onToggle }:
  { debts: DebtsData; onAdd:(d:Omit<DebtItem,'id'|'createdDate'|'paid'|'settled'>)=>void; onDelete:(id:string)=>void; onPay:(id:string,amount:number)=>void; onToggle:(id:string)=>void }) {
  const [kind, setKind] = useState<DebtKind>('debt')
  const [showAdd, setShowAdd] = useState(false)
  const [payingItem, setPayingItem] = useState<DebtItem|null>(null)

  const items = debts.items.filter(d=>d.kind===kind)
  const open = items.filter(d=>!d.settled)
  const settled = items.filter(d=>d.settled)
  const totalOpen = totalOutstanding(debts.items, kind)
  const color = kind==='debt' ? 'var(--bad)' : 'var(--good)'
  const Ico = kind==='debt' ? ArrowUp : ArrowDown

  return (
    <div className="space-y-4 slide-up">
      {/* Debt vs Credit switcher - same pattern as the category-manager kind toggle */}
      <div style={{display:'flex',gap:8}}>
        {(['debt','credit'] as const).map(k=>(
          <button key={k} onClick={()=>setKind(k)} className="tap" style={{flex:1,padding:'10px 0',borderRadius:'var(--r-field)',
            background:kind===k?(k==='debt'?'var(--bad-tint)':'var(--good-tint)'):'var(--surface-2)',
            border:`1.5px solid ${kind===k?(k==='debt'?'var(--bad)':'var(--good)'):'var(--border)'}`,
            fontSize:13,fontWeight:700,color:kind===k?(k==='debt'?'var(--bad)':'var(--good)'):'var(--t2)'}}>
            {k==='debt'?'Debts (I owe)':'Credits (owed to me)'}
          </button>
        ))}
      </div>
      ...
      <div className="glass" style={{padding:20,display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        <div>
          <p style={{fontSize:11,fontWeight:700,color:'var(--t3)',textTransform:'uppercase',letterSpacing:'0.07em'}}>{kind==='debt'?'Total You Owe':'Total Owed To You'}</p>
          <p className="f-display num" style={{fontSize:30,fontWeight:700,color,marginTop:2}}>
            {fmt(totalOpen)}<span style={{fontSize:14,fontWeight:400,color:'var(--t3)',marginLeft:4}}>MAD</span>
          </p>
          <p style={{fontSize:11,color:'var(--t3)',marginTop:3}}>{open.length} open · {settled.length} settled</p>
        </div>
        <button className="btn-primary tap" style={{width:'auto',padding:'10px 18px',fontSize:13}} onClick={()=>setShowAdd(true)}>
          <PlusCircle size={15} weight="bold"/>Add
        </button>
      ...
      {open.length>0&&(
        <div>
          <p style={{fontSize:11,fontWeight:700,color:'var(--t3)',textTransform:'uppercase',letterSpacing:'0.07em',padding:'0 4px',marginBottom:10}}>Open</p>
          <div style={{display:'flex',flexDirection:'column',gap:12}}>
            {open.map(item=>{
              const remaining = debtRemaining(item)
              const p = pct(item.paid, item.amount)
              return (
                <div key={item.id} className="glass" style={{padding:20}}>
                  <div style={{display:'flex',alignItems:'flex-start',gap:12}}>
                    <IconBadge Icon={Ico} color={color}/>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:8,marginBottom:4}}>
                        <p style={{fontSize:14,fontWeight:700,color:'var(--t1)'}}>{item.person}</p>
                        {item.dueDate && <Chip label={`Due ${item.dueDate}`} color={color}/>}
                      </div>
                      {item.note && <p style={{fontSize:12,color:'var(--t2)',marginBottom:8}}>{item.note}</p>}
                      <div style={{display:'flex',alignItems:'baseline',gap:6,marginBottom:10}}>
                        <span className="f-display num" style={{fontSize:20,fontWeight:700,color}}>{fmt(remaining)}</span>
                        <span style={{fontSize:12,color:'var(--t3)'}}>left of {fmt(item.amount)} MAD</span>
                      </div>
                      <PBar value={p} color={color} h={6}/>
                    </div>
                    <div style={{display:'flex',flexDirection:'column',gap:6,marginTop:2}}>
                      <button className="tap" onClick={()=>onToggle(item.id)} style={{color:'var(--t3)',padding:4}} title="Mark settled"><Check size={16} weight="bold"/></button>
                      <button className="tap" onClick={()=>setPayingItem(item)} style={{color,padding:4}} title={kind==='debt'?'Record a payment':'Record a collection'}><ArrowCircleUp size={16} weight="bold"/></button>
                      <button className="tap" onClick={()=>onDelete(item.id)} style={{color:'var(--t3)',padding:4}}><Trash size={13}/></button>
                    </div>
                  </div>
                </div>
      ...
      {settled.length>0&&(
        <div>
          <p style={{fontSize:11,fontWeight:700,color:'var(--t3)',textTransform:'uppercase',letterSpacing:'0.07em',padding:'0 4px',marginBottom:10}}>Settled</p>
          <div style={{display:'flex',flexDirection:'column',gap:8}}>
            {settled.map(item=>(
              <div key={item.id} className="glass" style={{padding:16,display:'flex',alignItems:'center',gap:12,opacity:.55}}>
                <button onClick={()=>onToggle(item.id)} className="tap"
                  style={{width:28,height:28,borderRadius:'50%',background:'var(--good)',flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center',border:'none'}}>
                  <Check size={13} weight="bold" color="#fff"/>
                </button>
                <div style={{flex:1}}>
                  <p style={{fontSize:13,fontWeight:600,color:'var(--t1)'}}>{item.person}</p>
                  <p style={{fontSize:11,color:'var(--t3)'}}>{fmt(item.amount)} MAD settled</p>
                </div>
                <button className="tap" onClick={()=>onDelete(item.id)} style={{color:'var(--t3)',padding:4}}><Trash size={13}/></button>
              </div>
            ))}
      ...
      {items.length===0&&(
        <div className="glass" style={{padding:40,textAlign:'center',color:'var(--t3)'}}>
          <HandCoins size={28} style={{margin:'0 auto 8px',opacity:.3}}/>
          <p style={{fontSize:13}}>{kind==='debt' ? 'No debts yet' : 'No credits yet'}</p>
        </div>
      )}

      {showAdd && <AddDebtModal kind={kind} onClose={()=>setShowAdd(false)} onAdd={onAdd}/>}
      {payingItem && <PayDebtModal item={payingItem} onClose={()=>setPayingItem(null)} onPay={(amount)=>onPay(payingItem.id, amount)}/>}
    </div>
  )
}

/* ══════════════════════ DASHBOARD ROOT ══════════════════════ */
type Tab = 'overview'|'variable'|'fixed'|'savings'|'debts'
...
export default function Dashboard() {
  ...
  // Debts & credits - global per user, same pattern as saving goals.
  useEffect(() => {
    if (!user) return
    if (profile && !profile.onboardingComplete) return
    const unsub = subscribeDebts(
      user.uid,
      s => setDebts(s),
      () => setError("Couldn't load your debts - check your connection or Firestore setup.")
    )
    return () => unsub()
  }, [user, profile])
  ...
  const persistDebts = useCallback(async (updated: DebtsData) => {
    if (!user) return
    setDebts(updated)
    setSaving(true)
    try {
      await saveDebts(user.uid, updated)
      setError('')
    } catch (e) {
      console.error(e)
      setError("Couldn't save your changes. Check your connection and try again.")
  ...
  // Debts & credits - simple ledger, doesn't move money between money
  // places (unlike savings) since these track what's owed, not where cash
  // physically sits.
  const addDebt = (d: Omit<DebtItem,'id'|'createdDate'|'paid'|'settled'>) => {
    if (!debts) return
    persistDebts({ items: [...debts.items, { ...d, id: uid2(), createdDate: today(), paid: 0, settled: false }] })
  }
  const delDebt = (id: string) => { if (debts && confirmDelete('this entry')) persistDebts({ items: debts.items.filter(d=>d.id!==id) }) }
  const recordPayment = (id: string, amount: number) => {
    if (!debts) return
    persistDebts({ items: debts.items.map(d => {
      if (d.id !== id) return d
      const paid = Math.min(d.amount, d.paid + amount)
      return { ...d, paid, settled: paid >= d.amount }
    }) })
  }
  const toggleSettled = (id: string) => debts && persistDebts({ items: debts.items.map(d=>d.id===id?{...d,settled:!d.settled}:d) })
  ...
  if (authLoading || !month || !savings || !debts) return (
          ...
          {tabs.map(t=>{
            ...
            return(
              <button key={t.id} onClick={()=>setTab(t.id)} className="tap"
                ...
                <t.Icon size={19} weight={active?'fill':'regular'}/>{t.label}
              </button>
            )
          })}
        </nav>
      ...
      <div className="flex flex-col flex-1 min-w-0 max-w-md md:max-w-none mx-auto md:mx-0" style={{background:'var(--bg)'}}>
        ...
        <div className="flex-1 overflow-y-auto px-4 pt-4 pb-28 md:px-8 md:pt-6 md:pb-10">
          <div className="md:max-w-5xl md:mx-auto">
            ...
            {tab==='overview' && <Overview month={month} savings={savings} debts={debts}/>}
            ...
            {tab==='debts'    && <DebtsTab debts={debts} onAdd={addDebt} onDelete={delDebt} onPay={recordPayment} onToggle={toggleSettled}/>}
          </div>
        </div>
            ...
            {mobileTabs.map(t=>{
              ...
              return(
                <button key={t.id} onClick={()=>setTab(t.id)} className="tap" aria-label={t.id}
                  ...
                  <t.Icon size={28} strokeWidth={active?2.25:1.6} color={active?'var(--t1)':'var(--t3)'} fill={active?'var(--t1)':'none'} fillOpacity={active?0.12:0}/>
                </button>
              )
            })}
          </div>
