    ...
    <div className="space-y-4 slide-up">
      ...
      <div className="relative" style={{borderRadius:'var(--r-card)',padding:24,background:'var(--surface)',border:'1px solid var(--border-2)',boxShadow:'var(--shadow-card)'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:10}}>
          {MONEY_PLACES.map(p=>(
            <div key={p} className="glass-2" style={{padding:'12px 10px'}}>
              <div className="flex items-center gap-1.5" style={{marginBottom:6}}>
                {(() => { const Ico = MONEY_PLACE_ICON[p]; return <Ico size={13} weight="bold" color="var(--t3)"/> })()}
...
/* ══════════════════════ SAVINGS ══════════════════════ */
function SavingsTab({month,onAdd,onDelete,onToggle,onTopUp}:{month:MonthBudget;onAdd:(g:Omit<SavingGoal,'id'>)=>void;onDelete:(id:string)=>void;onToggle:(id:string)=>void;onTopUp:(id:string,amount:number,place:MoneyPlace)=>void}){
  const [showAdd,setShowAdd]=useState(false)
...
export default function Dashboard() {
  ...
  // Funding a saving goal moves real money out of whichever money place it came from.
  const addSaving = (g: Omit<SavingGoal,'id'>) => {
    if (!month) return
    const patch = g.current > 0 ? withMoneyPlaceDelta(month, SOURCE_TO_PLACE[g.source], -g.current) : {}
    persist({ ...month, ...patch, savingGoals: [...month.savingGoals, { ...g, id: uid2() }] })
  }
  ...
  const topUpSaving = (id: string, amount: number, place: MoneyPlace) => {
    if (!month) return
    const patch = withMoneyPlaceDelta(month, place, -amount)
    persist({ ...month, ...patch, savingGoals: month.savingGoals.map(g => g.id===id ? {...g, current: g.current+amount, active: true} : g) })
  }
      ...
      <div className="flex flex-col flex-1 min-w-0 max-w-md md:max-w-none mx-auto md:mx-0" style={{background:'var(--bg)'}}>
        ...
        <div className="flex-1 overflow-y-auto px-4 pt-4 pb-28 md:px-8 md:pt-6 md:pb-10">
          <div className="md:max-w-5xl md:mx-auto">
            ...
            {tab==='savings'  && <SavingsTab month={month} onAdd={addSaving} onDelete={delSaving} onToggle={toggleSaving} onTopUp={topUpSaving}/>}
          </div>
        </div>
