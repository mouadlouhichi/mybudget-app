...
/* ── Debts & Credits ──────────────────────────────────────────────────────
   `debt`   = money you owe someone else (a liability).
   `credit` = money someone else owes you (an asset).
   Global per user, not per month - like saving goals, a debt doesn't reset
   just because the calendar rolled over; it stays open until it's settled. */
export type DebtKind = 'debt' | 'credit'
export interface DebtItem {
  id: string
  person: string
  amount: number       // original amount owed
  paid: number          // amount already repaid / collected
  kind: DebtKind
  note?: string
  dueDate?: string
  createdDate: string
...
export interface DebtsData { items: DebtItem[] }
export function emptyDebts(): DebtsData { return { items: [] } }
...
/* ── Debts & Credits helpers ── */
export function debtRemaining(d: DebtItem): number {
  return Math.max(0, d.amount - d.paid)
}
export function totalOutstanding(items: DebtItem[], kind: DebtKind): number {
  return items.filter(d => d.kind === kind && !d.settled).reduce((s, d) => s + debtRemaining(d), 0)
}
